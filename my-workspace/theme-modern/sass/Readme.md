...
folder are mapped to the application's root namespace, 'ThemeModern'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "ThemeModern"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'ThemeModern' in the same way
as `"ThemeModern/sass/src"`.
